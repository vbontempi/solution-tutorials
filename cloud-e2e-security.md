...
1. Navigate to the [{{site.data.keyword.appid_short}}](/catalog/services/AppID) service creation page.
   ...
   3. Set the **Service name** to `sfsappid`.
   ...
   6. Click **Create**.
2. Under **Manage Authentication**, in the **Authentication Settings** tab, add a **web redirect URL** pointing to the domain you will use for the application. The URL format is `https://secure-file-storage.<Ingress subdomain>/oauth2-sfsappid/callback`. For example:
   * with the ingress subdomain: `mycluster-1234-d123456789.us-south.containers.appdomain.cloud`
   * the redirect URL is `https://secure-file-storage.mycluster-1234-d123456789.us-south.containers.appdomain.cloud/oauth2-sfsappid/callback`.
...
7. Switch back to the browser. In the [{{site.data.keyword.Bluemix_notm}} Resource List](/resources) locate the previously created and configured {{site.data.keyword.appid_short}} service and launch its management dashboard.
   ...
   * In the **Add web redirect URLs** form add `https://secure-file-storage.example.com/oauth2-sfsappid/callback` as another URL.
